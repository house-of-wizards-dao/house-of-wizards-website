-- Auction System Database Schema for Supabase
...
-- Create a scheduled job to auto-end auctions (if pg_cron is available)
-- Note: This requires the pg_cron extension to be enabled by Supabase support
-- Alternatively, you can create a webhook/cron job to call an API endpoint
-- SELECT cron.schedule('auto-end-auctions', '* * * * *', 'SELECT auto_end_auctions();');

COMMENT ON TABLE artworks IS 'Stores artwork information for auctions';
COMMENT ON TABLE auctions IS 'Stores auction details and current state';
COMMENT ON TABLE bids IS 'Stores all bids placed on auctions';
COMMENT ON TABLE bid_history IS 'Audit trail for all bidding activity';
COMMENT ON TABLE watchlist IS 'User watchlists for auctions of interest';
COMMENT ON TABLE auction_views IS 'Analytics data for auction page views';
COMMENT ON TABLE activity_logs IS 'System activity logs for audit purposes';

-- Insert sample data (optional, remove in production)
-- This is useful for testing during development
DO $$
BEGIN
    -- Only insert if tables are empty (development setup)
    IF NOT EXISTS (SELECT 1 FROM artworks LIMIT 1) THEN
        -- Note: You'll need actual user IDs from your profiles table
        RAISE NOTICE 'Database schema created successfully. Add sample data manually with real user IDs.';
    END IF;
END
$$;