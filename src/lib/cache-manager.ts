...
export class CacheManager {
  ...
  private fallbackCache: Map<string, { value: any; expires: number; tags: string[] }> = new Map();
  ...
  private async initializeRedis() {
    ...
    try {
      ...
      logger.warn('Failed to connect to Redis, using fallback cache', { error });
      this.redis = null;
    }
  }
   ...
   */
  async cacheProfile(userId: string, profileData: any, ttl: number = 600): Promise<void> {
   ...
   */
  async cacheUserContent(userId: string, content: any[], ttl: number = 300): Promise<void> {
   ...
   */
  async cacheSearchResults(query: string, results: any[], ttl: number = 180): Promise<void> {
...
export function cached(key: string, options: CacheOptions = {}) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      return cacheManager.getOrSet(cacheKey, () => method.apply(this, args), options);
    };
  };
}