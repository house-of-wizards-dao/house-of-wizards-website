...
export class CSRFProtection {
   ...
   */
  public generateTokenForRequest(
    req: NextApiRequest,
    res: NextApiResponse,
  ): string {
  ...
  public validateToken(req: NextApiRequest): boolean {
    ...
    const requestToken =
      this.getTokenFromHeader(req) || this.getTokenFromBody(req);
    if (!requestToken) {
  ...
  public middleware() {
    return (req: NextApiRequest, res: NextApiResponse, next?: () => void) => {
      ...
      if (
        !this.parseCsrfCookie(req) &&
        !this.isStateMutatingMethod(req.method || "GET")
      ) {
